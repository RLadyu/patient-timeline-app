...
function renderTimeline() {
  ...
  let chartWidth = Math.max(MIN_WIDTH, baseWidth || MIN_WIDTH);
  let bottomMargin = calculateBottomMargin(chartWidth, visibleTrackKeys);
  let baseHeight = TOP_MARGIN + bottomMargin + 200;
  let chartHeight = lastTrack ? lastTrack.top + lastTrack.height + bottomMargin : baseHeight;

  for (let pass = 0; pass < 2; pass += 1) {
    const targetWidth = Math.ceil(chartHeight * LANDSCAPE_ASPECT_RATIO);
    if (chartWidth >= targetWidth) {
      break;
    }
    chartWidth = Math.max(chartWidth, targetWidth);
    bottomMargin = calculateBottomMargin(chartWidth, visibleTrackKeys);
    baseHeight = TOP_MARGIN + bottomMargin + 200;
    chartHeight = lastTrack ? lastTrack.top + lastTrack.height + bottomMargin : baseHeight;
  }
