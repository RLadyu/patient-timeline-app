...
function renderTherapy(track, dates, chartWidth, metrics) {
  ...
  intervals.forEach((item) => {
    ...
    const bounds = getTrackOffsetBounds(track, y, rectHeight);
    ...
    if (typeof totalDoses === 'number') {
      ...
      timelineSvg.appendChild(doseLabel);
      if (isFlagged) {
    ...
    registerDraggable(dragTargets, {
      ...
      getOffsetBounds: () => getTrackOffsetBounds(track, y, rectHeight),
      applyOffset: (nextOffset) =>
        applyTherapyOffsetOverride(
          item.courseId,
          item.medicationId,
          nextOffset,
          getTrackOffsetBounds(track, y, rectHeight)
        )
    });
...
function renderEndoscopy(track, dates, chartWidth, metrics) {
  ...
  items.forEach((item) => {
    ...
    timelineSvg.appendChild(dateMarker);
    ...
    if (interventionsText) {
      ...
      timelineSvg.appendChild(interventionsLabel);
    ...
    if (showComplicationsOnChart) {
      ...
      timelineSvg.appendChild(complicationLabel);
    }
    ...
    registerDraggable(dragTargets, {
      ...
      getOffsetBounds: () => getTrackOffsetBounds(track, y, rectHeight),
      applyOffset: (nextOffset) =>
        applyEndoscopyOffsetOverride(item.id, nextOffset, getTrackOffsetBounds(track, y, rectHeight))
    });
...
function renderLiver(track, dates, chartWidth) {
  ...
  const startY = track.top + (track.height - blockAreaHeight) / 2;
...
function appendFlagIndicator(x, y, options = {}) {
  ...
  timelineSvg.appendChild(indicator);
  return indicator;
}
