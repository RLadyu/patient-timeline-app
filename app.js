...
const state = {
  ...
  vitals: [
    {
      id: nextId('vital'),
      parameterId: 'bloodPressure',
      date: '2023-01-08',
      time: '09:05',
      systolic: '130',
      diastolic: '85',
      comment: 'Умеренное повышение утром',
      isFlagged: false
    },
    {
      id: nextId('vital'),
      ...
      time: '09:20',
      value: '96',
      comment: 'Тахикардия на фоне лихорадки',
      isFlagged: true
    },
    {
      id: nextId('vital'),
      ...
      time: '10:15',
      value: '94',
      comment: 'Требует кислородной поддержки',
      isFlagged: true
    },
    {
      id: nextId('vital'),
      ...
      systolic: '120',
      diastolic: '78',
      comment: 'Стабилизация показателей',
      isFlagged: false
    }
  ],
  therapy: [
...
const DETAIL_TYPE_COLORS = {
  ...
  'Клинические показатели': COLORS.vitals,
...
const SVG_STYLE_TEXT = `
  ...
  .vitals-card{fill:rgba(13,148,136,0.12);stroke:${COLORS.vitals};stroke-width:1.4;}
  .vitals-label{font-size:calc(12px * var(--font-scale));font-family:'Inter','Segoe UI',sans-serif;fill:#000000;}
  .vitals-marker{fill:${COLORS.vitals};stroke:#ffffff;stroke-width:2;}
...
const LEGEND_ITEMS = [
  ...
  { key: 'vitals', label: 'Клинические показатели' },
...
function findItem(type, id) {
  ...
  switch (type) {
    ...
    case 'vitals':
      return state.vitals.find((item) => item.id === id) || null;
    case 'event':
...
const PARAMETER_CATALOG = {
  clinical: [
    ...
    { id: 'bloodPressure', label: 'Артериальное давление', trackKey: 'vitals', formKind: 'bloodPressure' },
    { id: 'pulse', label: 'Пульс', trackKey: 'vitals', formKind: 'pulse' },
    { id: 'spo2', label: 'SpO2', trackKey: 'vitals', formKind: 'spo2' },
...
const formConfig = {
  ...
  bloodPressure: [
    { type: 'date', name: 'date', label: 'Дата', required: true },
    { type: 'time', name: 'time', label: 'Время', required: false, defaultValue: '09:00' },
    { type: 'number', name: 'systolic', label: 'Систолическое, мм рт. ст.', required: true, min: '60', max: '260' },
    { type: 'number', name: 'diastolic', label: 'Диастолическое, мм рт. ст.', required: true, min: '30', max: '160' },
  ...
  pulse: [
    { type: 'date', name: 'date', label: 'Дата', required: true },
    { type: 'time', name: 'time', label: 'Время', required: false, defaultValue: '09:00' },
    { type: 'number', name: 'value', label: 'Пульс, уд/мин', required: true, min: '20', max: '220' },
    { type: 'textarea', name: 'comment', label: 'Комментарий', required: false },
    {
      type: 'checkbox',
      name: 'isFlagged',
  ...
  spo2: [
    { type: 'date', name: 'date', label: 'Дата', required: true },
    { type: 'time', name: 'time', label: 'Время', required: false, defaultValue: '09:00' },
    { type: 'number', name: 'value', label: 'SpO2, %', required: true, min: '50', max: '100' },
    { type: 'textarea', name: 'comment', label: 'Комментарий', required: false },
    {
      type: 'checkbox',
      name: 'isFlagged',
...
const TOOLTIP_EDITORS = {
  ...
  vitals: {
    get(editInfo) {
      const item = state.vitals.find((entry) => entry.id === editInfo?.id);
      if (!item) return null;
      if (item.parameterId === 'bloodPressure') {
        return {
    ...
    fields: [
      { name: 'systolic', label: 'Систолическое, мм рт. ст.', type: 'number', min: '60', max: '260' },
      { name: 'diastolic', label: 'Диастолическое, мм рт. ст.', type: 'number', min: '30', max: '160' },
      { name: 'value', label: 'Значение', type: 'number', min: '20', max: '220' },
      { name: 'time', label: 'Время', type: 'time' },
      { name: 'comment', label: 'Комментарий', type: 'textarea' },
      { name: 'isFlagged', label: 'Проблемный участок', type: 'checkbox' }
    ],
    apply(editInfo, values) {
      ...
      let changed = false;
      if (item.parameterId === 'bloodPressure') {
        if (values.systolic !== undefined && values.systolic !== (item.systolic || '')) {
          item.systolic = values.systolic || '';
          changed = true;
        }
        if (values.diastolic !== undefined && values.diastolic !== (item.diastolic || '')) {
          item.diastolic = values.diastolic || '';
          changed = true;
        }
      } else if (values.value !== undefined && values.value !== (item.value || '')) {
      ...
      item.label = buildVitalsLabel(item);
      return changed;
    }
  },
  therapy: {
...
const TRACK_DEFINITIONS = [
  ...
  { key: 'vitals', label: 'Клинические показатели', minHeight: 64 },
...
const DEFAULT_LAYOUT_STATE = {
  stepX: STEP_X_DEFAULT,
  trackGap: TRACK_GAP,
  groupGap: GROUP_GAP_DEFAULT,
  groupGapOverride: { ...DEFAULT_GROUP_GAP_OVERRIDE },
  trackGapOverride: {},
  trackHeightOverride: {}
};
...
function cloneStateData(source = state) {
  return {
    ...
    vitals: Array.isArray(source.vitals) ? source.vitals.map((item) => ({ ...item })) : [],
...
function hasDataForTrackKey(key, source = state) {
  switch (key) {
    ...
    case 'vitals':
      return Array.isArray(source.vitals) && source.vitals.length > 0;
    case 'therapy':
      return (
...
function syncIdCounterWithState(source = state) {
  ...
  data.vitals.forEach((item) => consider(item.id));
...
function getEditingSnapshot(type) {
  ...
  switch (type) {
    ...
    case 'vitals': {
      const item = state.vitals.find((entry) => entry.id === editContext.id);
      if (!item) {
        return null;
      }
      const parameterId = editContext.parameterId || item.parameterId;
      if (parameterId === 'bloodPressure') {
        return {
          date: item.date || '',
          time: item.time || '',
          systolic: item.systolic || '',
...
function handleResetGraph() {
  ...
  renderStepXOverride = null;
  currentStepX = STEP_X_DEFAULT;
  state.layout = normalizeLayoutState({ ...DEFAULT_LAYOUT_STATE });
  saveLayoutToStorage(state.layout);
...
function updateTimelineDates() {
  ...
  if (isTrackVisible('vitals')) {
    state.vitals.forEach((item) => dates.add(item.date));
  }
...
function getStepX() {
  if (Number.isFinite(renderStepXOverride)) {
    return clampStepX(renderStepXOverride);
  }
...
function getTrackLayout(metricsByKey, visibleKeys, dates) {
  ...
  keys.forEach((key, index) => {
    ...
    } else if (track.key === 'vitals') {
      baseHeight = computeSingleDateTrackHeight(track.minHeight, metricsByKey?.vitals?.layout);
...
function formatVitalDisplayValue(item) {
  if (!item) {
    return '';
  }
  switch (item.parameterId) {
    case 'bloodPressure': {
      ...
      return value ? `${value} мм рт. ст.` : '';
    }
    case 'pulse':
      return item.value ? `${item.value} уд/мин` : '';
    case 'spo2':
...
function buildVitalsLabel(item) {
  const metricLabel = getParameterLabel(item.parameterId) || 'Клинические показатели';
  const value = formatVitalDisplayValue(item);
  return value ? `${metricLabel}: ${value}` : metricLabel;
}
...
function renderTimeline() {
  ...
  const radiologyMetrics = isTrackVisible('radiology')
    ...
    : { items: [], layout: { levelHeights: [], levelAssignments: new Map(), measurements: new Map() } };
  const vitalsMetrics = isTrackVisible('vitals')
    ? { items: [...state.vitals], layout: measureSingleDateCardLayout(state.vitals, dates, 'vitals') }
    : { items: [], layout: { levelHeights: [], levelAssignments: new Map(), measurements: new Map() } };
  ...
  const computeChartSizing = () => {
    ...
    if (endoscopyMetrics.layout && Array.isArray(endoscopyMetrics.items)) {
      endoscopyMetrics.items.forEach((item) => {
        ...
        const width = Math.max(
          minWidth,
          (measurement?.maxChars || 32) * (SVG_CHAR_WIDTH * Math.max(fontScale, 0.85)) + 48,
          240
        );
        const center = getXPosition(item.date, dates);
    ...
    const expandForSingleDate = (metrics) => {
    ...
    expandForSingleDate(vitalsMetrics);
    expandForSingleDate(neuroMetrics);
    expandForSingleDate(labMetrics);
    expandForSingleDate(eventMetrics);
    expandForSingleDate(surgeryMetrics);
    expandForSingleDate(radiologyMetrics);
    ...
    return { chartWidth, bottomMargin, baseHeight, chartHeight };
  };

  renderStepXOverride = null;
  let sizing = computeChartSizing();
  if (dateSpan > 0) {
    const targetWidth = Math.ceil(sizing.chartHeight * LANDSCAPE_ASPECT_RATIO);
    const requiredStep = (targetWidth - LEFT_MARGIN - CHART_RIGHT_PAD) / dateSpan;
    const requiredStepX = clampStepX(requiredStep);
    if (requiredStepX > getStepX()) {
      renderStepXOverride = requiredStepX;
      sizing = computeChartSizing();
    }
  }

  const { chartWidth, bottomMargin, chartHeight } = sizing;
  ...
  renderVitals(tracks.find((track) => track.key === 'vitals'), dates, chartWidth, vitalsMetrics);
...
function renderVitals(track, dates, chartWidth, metrics) {
  const renderTrack = getTrackContentTrack(track);
  const layer = getTrackContentLayer(track);
  if (!renderTrack || !layer) return;
  const sorted = [...state.vitals].sort((a, b) => parseDateTime(a.date, a.time) - parseDateTime(b.date, b.time));
  if (!sorted.length) return;
  ...
  sorted.forEach((item) => {
    ...
    const fontScale = getEffectiveChartFontScale(item);
    const level = layout.levelAssignments?.get(item.id) ?? item.__level ?? 0;
    const labelText = buildVitalsLabel(item);
    const cardResult = renderCardWithText({
      layer,
      track: renderTrack,
      item,
      ...
      fontScale,
      text: labelText,
      classRect: 'vitals-card',
      classText: 'vitals-label'
    });
    if (!cardResult) {
      return;
    }
    ...
    const detail = {
      type: 'Клинические показатели',
      date: item.date,
      time: item.time,
      title: getParameterLabel(item.parameterId) || 'Клинические показатели',
      comment: item.comment || '',
      color: COLORS.vitals,
      isFlagged: Boolean(item.isFlagged),
      deleteInfo: { type: 'vitals', id: item.id },
      fontTarget: { type: 'vitals', id: item.id },
      iconContext: { type: 'vitals', id: item.id }
    };

    const editInfo = { type: 'vitals', id: item.id, parameterId: item.parameterId };
    attachDetails(marker, detail, marker, item.id, editInfo);
    renderItemIcon({
      layer,
      type: 'vitals',
...
function applyImportedState(nextState, options = {}) {
  ...
  state.vitals = Array.isArray(nextState.vitals) ? nextState.vitals : [];
...
function buildVitalsPayload(data, parameterId) {
  const base = {
    parameterId,
    date: data.date,
    time: data.time || '',
...
function handleFormSubmit(event) {
  ...
  if (isEditing) {
    switch (type) {
      ...
      case 'vitals': {
        const target = state.vitals.find((item) => item.id === editContext.id);
        if (target) {
          Object.assign(target, buildVitalsPayload(data, parameterId));
        }
        break;
      }
      case 'therapy': {
    ...
    switch (type) {
      ...
      case 'vitals':
        state.vitals.push({
          id: nextId('vital'),
          ...buildVitalsPayload(data, parameterId)
        });
        break;
      case 'therapy': {
...
function deleteEntity(context) {
  ...
  switch (context.type) {
    ...
    case 'vitals': {
      const index = state.vitals.findIndex((item) => item.id === context.id);
      if (index === -1) return false;
...
async function exportSvgAsPng(svgElement, options = {}) {
  ...
  const scale = Number.isFinite(options.scale) ? Math.max(1, options.scale) : window.devicePixelRatio > 1 ? window.devicePixelRatio : 1;
...
async function handleDownload() {
  ...
  try {
    renderTimeline();
    const result = await exportSvgAsPng(timelineSvg, { mode: 'full', scale: EXPORT_PNG_SCALE });
    if (!result) {
